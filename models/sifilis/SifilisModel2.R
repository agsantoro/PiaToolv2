...
procesarResultados <- function(basal, proyectado) {
  ...
  interpretacionDecision <- function(deltaCosto, deltaBeneficio) {
    ...
    if (deltaCosto == 0 & deltaBeneficio == 0) return(list(valor = "-", info = "")) #[06/02 LJP]
...
correrModelo <- function(parametros) {
  ...
  #[06/02 LJP]
  calcularEventos <- function(gestantes, tieneSifilis, pTratados, parametros, pMortinatoCrudo, pMNeonatalCrudo) {
    #%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if (tieneSifilis) {
      #Calculamos las probabilidades de mortinato y muerte neonatal segun la efectividad de la penicilina
      # sobre la fraccion atribuible a sifilis
      pMortinato <- pMortinatoCrudo * parametros$pMortinatoSG
      pMuerteNeonatal <- pMNeonatalCrudo * parametros$pMuerteNeonatalSG 
      pMortinatoTTO <- pMortinatoCrudo * ((parametros$pMortinatoSG - 1) * (1 - parametros$efPenicilinaMortinato) + 1)
      pMuerteNeonatalTTO <- pMNeonatalCrudo * ((parametros$pMuerteNeonatalSG - 1) * (1 - parametros$efPenicilinaMuerteNeonatal) + 1)
      ...
      pMortinato <- pMortinatoCrudo
      pMuerteNeonatal <- pMNeonatalCrudo
      pMortinatoTTO <- 0
    ...
    #print(paste("Mortinato Sin Sifilis:", pMortinatoCrudo, "Mortinato Con Sifilis Sin TTO:", pMortinato, "Mortinato Con Sifilis Con TTO:", pMortinatoTTO))
    #print(paste("pMNeonatalCrudo Sin Sifilis:", pMNeonatalCrudo, "pMNeonatalCrudo Con Sifilis Sin TTO:", pMuerteNeonatal, "pMNeonatalCrudo Con Sifilis Con TTO:", pMuerteNeonatalTTO))
  ...
  calcularResultados <- function(pTesteo, pTratamiento, cCostoTest, cCostoTTOGestante, sensibilidad, especificidad, nacimientos, nacimientosAtendidos, sifilisGestacionales, sifilisGestacionalesAtendidas, parametros, pMortinatoCrudo, pMNeonatalCrudo, bTestRapido) { #[06/02 LJP]
    #%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    sinSifilisNoAtendidos <- calcularEventos(gestantes =  (nacimientosNoAtendidos - sifilisGestacionalesNoAtendidas), tieneSifilis = 0, pTratados = 0, parametros = parametros, pMortinatoCrudo = pMortinatoCrudo, pMNeonatalCrudo = pMNeonatalCrudo)

    #Nacimientos no atendidos con sifilis
    sifilisNoAtendidos <- calcularEventos(gestantes = sifilisGestacionalesNoAtendidas, tieneSifilis = 1, pTratados =  0, parametros = parametros, pMortinatoCrudo = pMortinatoCrudo, pMNeonatalCrudo = pMNeonatalCrudo)

    #Nacimientos atendidos pero no Testeados sin Sifilis
    sinSifilisAtendidosNoTesteados <- calcularEventos(gestantes = (nacimientosAtendidos - sifilisGestacionalesAtendidas) * (1 - pTesteo), tieneSifilis =  0, pTratados = 0, parametros = parametros, pMortinatoCrudo = pMortinatoCrudo, pMNeonatalCrudo = pMNeonatalCrudo)

    #Nacimientos atendidos pero no Testeados con Sifilis
    sifilisAtendidosNoTesteados <- calcularEventos(gestantes = sifilisGestacionalesAtendidas * (1 - pTesteo), tieneSifilis =  1, pTratados = 0, parametros = parametros, pMortinatoCrudo = pMortinatoCrudo, pMNeonatalCrudo = pMNeonatalCrudo)
    ...
    sifilisTestPositivos <- calcularEventos(gestantes = sifilisGestacionalesAtendidas * pTesteo * sensibilidad, tieneSifilis = 1, pTratados =  pTratamiento, parametros = parametros, pMortinatoCrudo = pMortinatoCrudo, pMNeonatalCrudo = pMNeonatalCrudo)
      #Test Negativos, no tratados
    sifilisTestNegativos <- calcularEventos(gestantes = sifilisGestacionalesAtendidas * pTesteo *  (1 - sensibilidad), tieneSifilis = 1 , pTratados = 0, parametros = parametros, pMortinatoCrudo = pMortinatoCrudo, pMNeonatalCrudo = pMNeonatalCrudo)
    
    
      #Test Positivos
    sinSifilisTestPositivos <- calcularEventos(gestantes = (nacimientosAtendidos - sifilisGestacionalesAtendidas) * pTesteo * (1 - especificidad), tieneSifilis = 0, pTratados = 0, parametros = parametros, pMortinatoCrudo = pMortinatoCrudo, pMNeonatalCrudo = pMNeonatalCrudo)
      #Test Negativos
    sinSifilisTestNegativos <- calcularEventos(gestantes = (nacimientosAtendidos - sifilisGestacionalesAtendidas) * pTesteo * especificidad, tieneSifilis = 0, pTratados = 0, parametros = parametros, pMortinatoCrudo = pMortinatoCrudo, pMNeonatalCrudo = pMNeonatalCrudo)
    ...
    #%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    #[06/02 LJP]
    costosTesteo <- nGestantesTesteadas * cCostoTest * 2
    
    if (bTestRapido == 1)
    {
      costosTesteo <- costosTesteo + (
        sifilisGestacionalesAtendidas * pTesteo * sensibilidad +
        (nacimientosAtendidos - sifilisGestacionalesAtendidas) * pTesteo * (1 - especificidad)
      ) * parametros$cLNTP
    }
    #costosTratamiento <- nGestantesTratadas * parametros$cPeniBenza * 3
    costosTratamiento <- nGestantesCorrectamenteTratadas * parametros$cPeniBenza * 3 + nGestantesIncorrectamenteTratadas* parametros$cPeniBenza
    #[/06/02 LJP]
  ...
  nSinSifilis <- (nNacimientosTotales - nSifilisGestacional)
  nSifilisTratadas <- (nSifilisGestacionalAtendidas * parametros$pSGTratadas * parametros$pTesteoSGestacional)
  nSifilisNoTratadas <- nSifilisGestacional - nSifilisTratadas
  
  #print(paste("Sin Sifilis", nSinSifilis, "Sifilis Tratadas", nSifilisTratadas, "Sifilis No Tratadas", nSifilisNoTratadas))
  
  pCrudeMortinato <- (pMortinato * nNacimientosTotales) / (nSinSifilis + (nSifilisTratadas * ((parametros$pMortinatoSG - 1) * (1 - parametros$efPenicilinaMortinato) + 1)) + nSifilisNoTratadas * parametros$pMortinatoSG)
  
  pCrudeNeonatal <- (pMuerteNeonatal * nNacimientosTotales) / (nSinSifilis + (nSifilisTratadas * ((parametros$pMuerteNeonatalSG - 1) * (1 - parametros$efPenicilinaMuerteNeonatal) + 1)) + nSifilisNoTratadas * parametros$pMuerteNeonatalSG)
  
  
  #print(paste("El pMortinatoCrudo:", pCrudeMortinato, "El MuerteNeonatal Crudo", pCrudeNeonatal))

  
  resultadosBasal <- calcularResultados(parametros$pTesteoSGestacional, parametros$pSGTratadas, parametros$cLNTP + parametros$cLTP, parametros$cPeniBenzatinica, 1, 1, nNacimientosTotales, nNacimientosAtendidos, nSifilisGestacional, nSifilisGestacionalAtendidas, parametros, pCrudeMortinato, pCrudeNeonatal, bTestRapido = 0)

  resultadosProyectados <- calcularResultados(parametros$pTesteoSGestacionalObj, parametros$pSGTratadasObj, parametros$cRTP, parametros$cPeniBenzatinica, parametros$pRTPSensibilidad, parametros$pRTPEspecificidad, nNacimientosTotales, nNacimientosAtendidos, nSifilisGestacional, nSifilisGestacionalAtendidas, parametros, pCrudeMortinato, pCrudeNeonatal, bTestRapido = 1)
  #[/06/02 LJP]
...
cargar <- function(country) {
  data <- read_excel("models/sifilis/data/lparametros2.xlsx", sheet = "parametros", col_types = c("text","text","text","text","numeric","skip","skip","skip","skip","skip"))
...
sifilisInputList = function() {
  data <- read_excel("models/sifilis/data/lparametros2.xlsx", sheet = "Clasificacion Parametros", col_names = F, )
