...
hpv_page <- div(
  ...
  tags$script(HTML("
  $(document).on('shinyjs:ready', function() {
    // Usar delegación de eventos para asegurar que funcione
    $(document).on('click', '#toggle_floating_buttons_hpv', function(e) {
      e.preventDefault();
      e.stopPropagation();
      
      var container = $('#floatingButtonsContainer');
...
")),
  ...
  div(
    ...
    actionButton(
      ...
      actionButton(
        inputId = "toggle_floating_buttons_hpv",
        label = NULL,
