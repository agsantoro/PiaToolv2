...
formatear_pesos_naat <- function(x, decimales = 0) {
  if (is.numeric(x)){
...
formatear_pesos2_naat <- function(x, decimales = 0) {
  if (is.numeric(x)){
    return(paste0("$", formatC(x, format = "f", big.mark = ".", decimal.mark = ",", digits = decimales)))} else {
      return(x)
    }
}
formatear_epi_naat <- function(x, decimales = 2) {
  formatC(x, format = "f", big.mark = ".", decimal.mark = ",", digits = decimales)
}
formatear_porcentaje_naat <- function(x, decimales = 0) {
  if (is.numeric(x)){
...
procesarResultados_naat <- function(basal, proyectado) {
  ...
  indicadores <- list(deltaExitosos = formatear_epi_naat(proyectado$tratamientosExitosos - basal$tratamientosExitosos),
                      deltaMuertes = formatear_epi_naat(proyectado$muertes - basal$muertes),
                      deltaLTFU = formatear_epi_naat(proyectado$ltfu - basal$ltfu),
                      deltaLyLost = formatear_epi_naat(proyectado$lyLost - basal$lyLost) , 
                      deltaCostoTesteo = formatear_pesos2_naat(proyectado$costoTesteo - basal$costoTesteo),
                      deltaOtrosCostos = formatear_pesos2_naat(proyectado$otrosCostos - basal$otrosCostos)
  )
  
  #Eventos sanitarios
  tabla_sanitaria <- data.frame(
    Categorias = c("Tratamientos Exitosos", "Muertes", "LTFU", "Años Perdidos", "Disutilidad", "Dalys"),
    Escenario_Actual = c(basal$tratamientosExitosos, basal$muertes, basal$ltfu, basal$lyLost, basal$disLost, basal$dalys),
    Escenario_Proyectado = c(proyectado$tratamientosExitosos, proyectado$muertes, proyectado$ltfu, proyectado$lyLost, proyectado$disLost, proyectado$dalys),
    Diferencia =  c(proyectado$tratamientosExitosos - basal$tratamientosExitosos, proyectado$muertes - basal$muertes, proyectado$ltfu - basal$ltfu, proyectado$lyLost - basal$lyLost, proyectado$disLost - basal$disLost, proyectado$dalys - basal$dalys)
  )
  #Formateamos la tabla
  tabla_sanitaria[] <- lapply(tabla_sanitaria, function(col) {
    if (is.numeric(col)) formatear_epi_naat(col) else col
  })
  ...
  tabla_costos[] <- lapply(tabla_costos, function(col) {
    if (is.numeric(col)) formatear_pesos2_naat(col) else col
  })
  #Nombramos Headers
  ...
  func_format <- c(
    ...
    formatear_epi_naat,
    formatear_epi_naat,
    formatear_pesos2_naat,
    formatear_pesos2_naat,
    formatear_pesos2_naat,
    formatear_pesos2_naat,
    formatear_pesos2_naat,
    formatear_porcentaje_naat
  )
...
estimarPayoffsCostos_naat <- function(desenlaces, parametros, pMuerte, pMuerteMDR, tasaDescuento) {
  ...
  Total <- cAdecuadosSensibles + cInadecuadosSensibles + cAdecuadosResistentes + cAdecuadosResistentesPreXDR + cInadecuadosResistentes + cInadecuadosResistentesPreXDR + cTardiosSensibles + cTardiosMDR + cTardiosPreXDR #  + cInnecesarios + cInnecesariosMDR 
...
estimarDesenlaces_naat <- function(sospechosos, confirmados, sensibilidad, especificidad, sensibilidadResistencia, especificidadResistencia = 1, pEmpirico, pResistencia, pPreXDR) {
  ...
  adecuadosResistentes <- verdaderosPositivos * pResistencia * sensibilidadResistencia * (1 - pcPreXDR)
  
  #Tuberculosis que iniciaron tratamiento para MDR pero son PreXDR
  adecuadosResistentesPreXDR <- verdaderosPositivos * pResistencia * sensibilidadResistencia * pcPreXDR
  
  #Son tuberculosis resistentes pero les dio negativa la sensibilidad
  inadecuadosResistentes <- verdaderosPositivos * pResistencia * (1 - sensibilidadResistencia) * (1 - pcPreXDR) + falsosNegativos_Empirico * (1 - pcPreXDR) * pResistencia
  
  #Son tuberculosis resistentes pero les dio negativa la sensibilidad
  inadecuadosResistentesPreXDR <- verdaderosPositivos * pResistencia * (1 - sensibilidadResistencia) * pcPreXDR  + falsosNegativos_Empirico * pcPreXDR * pResistencia
  ...
  tardiosMDR <- tardios * pResistencia * (1 - pcPreXDR)
  #[/06/02 LJP]
...
naatInputList = function() {
  ...
  data = labels %>% left_join(data) 
  data$tipo[is.na(data$tipo)] = "Avanzado"
